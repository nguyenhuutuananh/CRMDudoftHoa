...
class Votes_model extends CRM_Model
{
    ...
    public function insert_votes_contract($id,$data)
    {
    ...
    public function update_votes_cotract($id_votes,$data=array())
    {
        $ass=0;
        $_array_id=array();
        foreach($data as $rom)
        ...
        $this->db->delete('tblvotes_contract');
        if($this->db->affected_rows() > 0){
            $ass++;
        }

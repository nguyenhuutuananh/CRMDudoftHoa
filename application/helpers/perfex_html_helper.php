...
function init_relation_call_logs_table($table_attributes = array())
{
    $table_data = array(
        _l('lead_number'),
        _l('lead_people_call'),
        _l('lead_people_date_call'),
        _l('lead_time_width'),
//        _l('lead_number'),
        _l('lead_assignment'),
        _l('lead_checkout')
    ...
    $table_data = do_action('call_logs_related_table_columns',$table_data);
...
// cong text
function render_datatable2($headings = array(), $class = '', $additional_classes = array(''), $table_attributes = array(), $tfoot = false)
{
    $_additional_classes = '';
    $_table_attributes   = ' ';
    if (count($additional_classes) > 0) {
...
function format_status_adjustment($id)
{

        $label = get_status_label($id);
        if ($id == 2) {
            $label = 'light-green';
            $status_name="Phiếu điều chỉnh kho đã duyệt";
        }
        else if ($id == 1) {
                $label = 'info';
                $status_name="Phiếu điều chỉnh kho được xác nhận";
            } else if ($id == 0) {
                $label = 'warning';
                $status_name="Phiếu điều chỉnh kho chưa được xác nhận";
            }
            $class = 'label label-' . $label;
            return '<span class="inline-block ' . $class . '">' . $status_name . '</span>';
}
